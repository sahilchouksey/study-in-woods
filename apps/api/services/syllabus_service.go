...
func (s *SyllabusService) saveMultiSubjectSyllabusData(ctx context.Context, document *model.Document, data *SyllabusExtractionResult, rawResponse string) ([]*model.Syllabus, error) {
	...
	if s.subjectService != nil && len(subjectsNeedingAISetup) > 0 {
		log.Printf("SyllabusService: Starting AI setup for %d subjects after transaction commit", len(subjectsNeedingAISetup))
		for _, subjectID := range subjectsNeedingAISetup {
			go func(sid uint) {
				ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
				defer cancel()
				if result, err := s.subjectService.SetupSubjectAI(ctx, sid); err != nil {
					log.Printf("Warning: SyllabusService failed to setup AI for subject %d: %v", sid, err)
				} else {
